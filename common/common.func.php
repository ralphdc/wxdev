...
function prepare_wx_data($msg_obj , $db , $logger , &$id)
{
	if($msg_type = $msg_obj->MsgType)
	{
		switch ($msg_type) {
			case 'text':
				# code...
				$sql = "insert into wx_history(tousername, fromusername, createtime, msgtype, msgid, content) values ('".$msg_obj->ToUserName."','".$msg_obj->FromUserName."','".$msg_obj->MsgType."','".$msg_obj->MsgId."','".$msg_obj->Content."')";
				break;
			case 'image':
				# code...
				break;
